...
plotStockChange <- function(data, country = 'Austria',indicator = 'Inward',years = 2007:2023,plot = "interactive") {
  ...
  title_txt <- paste0(country, " — ", indicator, " FDI (StockChange = Flow + OtherChange)")
  
  if (plot == "interactive") {
    plot_ly(bars, x = ~Year, y = ~Value,
            type = "bar",
            color = ~Component,
            colors = c("Flow" = "#1f77b4", "OtherChange" = "#ff7f0e")
    ) %>%
      add_trace(
        data = dataplot[!is.na(StockChange)],
    ...
    ggplot() +
      geom_col(data = bars, aes(Year, Value, fill = Component)) +
      geom_line(data = dataplot[!is.na(StockChange)],
                aes(Year, StockChange, color = "StockChange"), linewidth = 0.8) +
      geom_point(data = dataplot[!is.na(StockChange)],
                 aes(Year, StockChange, color = "StockChange"), size = 2) +
      scale_fill_manual(values = c(Flow = "#1f77b4", OtherChange = "#ff7f0e")) +
      scale_color_manual(values = c(StockChange = "#2ca02c"), name = "") +
      guides(fill = guide_legend(order = 1), color = guide_legend(order = 2)) +
      theme_minimal(base_size = 12) +
      labs(title = title_txt, x = "Year", y = paste0(indicator, " (flow/stock change)"), fill = "Component")+
